	...
	var daoFactory = this.findDAOFactory(modelName);
  	...
  	if(!!query){
		...
		for (var key in jsonQuery) { //loop through the keys
		    ...
		    } else if (key.indexOf("_like") > -1){ // Like
		        var fieldName = key.split("_like")[0];
		        likeFields.push({
		            field_name: fieldName,
		            field_value: jsonQuery[key]
		        });
		    } else if (key.indexOf("_lte") > -1){ // Lower than or equal
		        var fieldName = key.split("_lte")[0];
		        lteFields.push({
		            field_name: fieldName,
		            field_value: jsonQuery[key]
		        });
		    } else if (key.indexOf("_lt") > -1){ // Lower than
		        var fieldName = key.split("_lt")[0];
		        ltFields.push({
		            field_name: fieldName,
		            field_value: jsonQuery[key]
		        });
		    } else if (key.indexOf("_gte") > -1){ // Greater than or equal
		        var fieldName = key.split("_gte")[0];
		        gteFields.push({
		            field_name: fieldName,
		            field_value: jsonQuery[key]
		        });
		    } else if (key.indexOf("_gt") > -1){ // Greater than
		        var fieldName = key.split("_gt")[0];
		        gtFields.push({
		            field_name: fieldName,
		            field_value: jsonQuery[key]
		        });
		    } else if (key.indexOf("_ne") > -1){ // Not equal
		        var fieldName = key.split("_ne")[0];
		        neFields.push({
		            field_name: fieldName,
		            field_value: jsonQuery[key]
		        });
		...
		if (lteFields.length > 0){
		    for (var i = 0; i < lteFields.length; i++){
		        delete jsonQuery[lteFields[i].field_name + '_lte'];
		...
		if (ltFields.length > 0){
		    for (var i = 0; i < ltFields.length; i++){
		        delete jsonQuery[ltFields[i].field_name + '_lt'];
		...
		if (gteFields.length > 0){
		    for (var i = 0; i < gteFields.length; i++){
		        delete jsonQuery[gteFields[i].field_name + '_gte'];
		...
		if (gtFields.length > 0){
		    for (var i = 0; i < gtFields.length; i++){
		        delete jsonQuery[gtFields[i].field_name + '_gt'];
		...
		if (neFields.length > 0){
		    for (var i = 0; i < neFields.length; i++){
		        delete jsonQuery[neFields[i].field_name + '_ne'];
